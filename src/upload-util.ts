...
export function commitAWSDir(
	version: string,
	sha: string,
	s3Config: Interfaces.S3,
): string {
...
export function templateShortKey(
	...
	const templates = {
		...
		manifest:
			"<%- bin %>-v<%- version %>-<%- sha %>-<%- platform %>-<%- arch %>-buildmanifest",
		unversioned: "<%- bin %>-<%- platform %>-<%- arch %><%- ext %>",
		versioned:
			"<%- bin %>-v<%- version %>-<%- sha %>-<%- platform %>-<%- arch %><%- ext %>",
