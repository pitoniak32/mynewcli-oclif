...
import {BuildConfig as TarballConfig} from './tarballs/config.js'

export function commitAWSDir(version: string, sha: string, s3Config: Interfaces.S3): string {
  let s3SubDir = s3Config.folder || ''
  if (s3SubDir !== '' && s3SubDir.slice(-1) !== '/') s3SubDir = `${s3SubDir}/`
  return join(s3SubDir, 'versions', version, sha)
}

export function channelAWSDir(channel: string, s3Config: Interfaces.S3): string {
  let s3SubDir = s3Config.folder || ''
  if (s3SubDir !== '' && s3SubDir.slice(-1) !== '/') s3SubDir = `${s3SubDir}/`
