...
import { CopyResponse, Storage } from "@google-cloud/storage";
...
export default class Promote extends Command {
	...
	static flags = {
		...
		"dry-run": Flags.boolean({
			description:
				"Run the command without uploading to S3 or copying versioned tarballs/installers to channel.",
		}),
		"ignore-missing": Flags.boolean({
			description:
				"Ignore missing tarballs/installers and continue promoting the rest.",
		}),
	...
	async run(): Promise<void> {
		...
		this.log(
			`Promoting v${flags.version} (${flags.sha}) to ${flags.channel} channel\n`,
		);
		...
		const cloudChannelKey = (shortKey: string) =>
			path.join(channelAWSDir(flags.channel, s3Config), shortKey);

		// copy tarballs manifests
		const promoteManifest = async (
			...
			const unversionedManifest = manifest.replace(
				`-v${flags.version}-${flags.sha}`,
				"",
			);

			const srcLocation = cloudBucketCommitKey(manifest)
			const dstLocation = cloudChannelKey(unversionedManifest)
			...
			if (!flags["dry-run"]) {
				...
				log(`[DRY_RUN] skipping manifest upload: ${srcLocation} -> ${dstLocation}`)
			}

		};

		const promoteGzTarballs = async (
			target: (typeof buildConfig.targets)[number],
		) => {
			...
			const unversionedTarGzName = versionedTarGzName.replace(
				`-v${flags.version}-${flags.sha}`,
				"",
			);
			const unversionedTarGzKey = cloudChannelKey(unversionedTarGzName);

			const srcLocation = versionedTarGzKey
			const dstLocation = unversionedTarGzKey

			let uploadPromise: Promise<CopyResponse> | undefined = undefined
			if (!flags["dry-run"]) {
				uploadPromise = storage
					.bucket(s3Config.bucket!)
					.file(srcLocation)
					.copy(storage.bucket(s3Config.bucket!).file(dstLocation))
			} else {
				log(`[DRY_RUN] skipping tarball upload: ${srcLocation} -> ${dstLocation}`)
			}
