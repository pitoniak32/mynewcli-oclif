...
import { BuildConfig } from './tarballs/index.js'
...
export const appendToIndex = async (input: {
  ...
  try {
    ...
    const contents = await storage.bucket(s3Config.bucket).file(key).download()
  ...
  writeFileSync(jsonFileName, JSON.stringify(sortVersionsObjectByKeysDesc(
      {
        ...existing,
  ...
  await storage.bucket(s3Config.bucket).upload(jsonFileName, options)
