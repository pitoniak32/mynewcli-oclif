...
import { copy, emptyDir, move, remove } from "fs-extra";
...
const pack = async (from: string, to: string) => {
	...
	log(
		`packing tarball from ${prettifyPaths(path.dirname(from))} to ${prettifyPaths(to)}`,
	);
	if (to.endsWith("gz")) {
		return exec(
			`tar czf ${to} ${path.basename(from)}${process.platform === "win32" ? " --force-local" : ""}`,
			{
				cwd,
			},
		);
	...
	await exec(
		`tar cfJ ${to} ${path.basename(from)}${process.platform === "win32" ? " --force-local" : ""}`,
		{ cwd },
	);
};
...
const copyCoreYarnFiles = async (
	...
	const workspaceYarnConfigFilePath = path.join(
		workspacePath,
		yarnConfigFileName,
	);
...
export async function build(
	...
	if (!c.updateConfig.s3?.host || !c.updateConfig.s3?.bucket) {
		ux.warn(
			"No bucket or host configured. CLI will not be able to update itself.",
		);
	}

	const targetsToBuild = c.targets.filter(
		(t) => !options.platform || options.platform === t.platform,
	);
	if (options.parallel) {
...
const removeLockfiles = async (c: BuildConfig) => {
	...
	const lockfiles = files
		.filter((f) => isLockFile(f))
		.map((f) => path.join(c.workspace(), f));
...
const pretarball = async (c: BuildConfig) => {
	const pjson = JSON.parse(
		readFileSync(path.join(c.workspace(), "package.json"), "utf8"),
	);
...
const addDependencies = async (c: BuildConfig) => {
	...
	if (isYarnProject(yarnRoot)) {
		...
		if (yarnVersion === "1") {
			await exec("yarn --no-progress --production --non-interactive", {
				cwd: c.workspace(),
			});
		} else if (yarnVersion === "2") {
			throw new Error(
				"Yarn 2 is not supported yet. Try using Yarn 1, or Yarn 3",
			);
		} else {
			try {
				await exec("yarn workspaces focus --production", {
					cwd: c.workspace(),
				});
			} catch (error: unknown) {
				if (
					...
					throw new Error(
						"Missing workspace tools. Run `yarn plugin import workspace-tools`.",
					);
				}
		...
		await copy(
			path.join(c.root, "pnpm-lock.yaml"),
			path.join(c.workspace(), "pnpm-lock.yaml"),
		);
...
const extractCLI = async (tarball: string, c: BuildConfig) => {
	...
	const files = await readdir(path.join(workspace, "package"), {
		withFileTypes: true,
	});
	await Promise.all(
		files.map((i) =>
			move(
				path.join(workspace, "package", i.name),
				path.join(workspace, i.name),
			),
		),
	);
...
const buildTarget = async (
	...
	if (options.parallel) {
		await Promise.all([
			...
			...(c.xz ? [pack(workspace, c.dist(xzLocalKey))] : []),
		]);
	} else {
	...
	const manifest: Interfaces.S3Manifest = {
		...
		gz: s3Url(gzCloudKey, c.config.updateConfig.s3),
		node: {
	...
	const manifestFilepath = c.dist(
		templateShortKey("manifest", templateShortKeyCommonOptions),
	);
	writeFileSync(manifestFilepath, JSON.stringify(manifest, null, 2));
};

function s3Url(key: string, updateConfig?: Interfaces.S3): string {
	const { host } = updateConfig ?? { host: undefined };
