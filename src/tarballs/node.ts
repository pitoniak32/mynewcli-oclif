...
export async function fetchNodeBinary({
	...
	let cache = path.join(
		tmp,
		"cache",
		`node-v${nodeVersion}-${platform}-${arch}`,
	);
	...
	const download = async () => {
		...
		if (!existsSync(shasums)) {
			await pipeline(
				got.stream(
					`https://nodejs.org/dist/v${nodeVersion}/SHASUMS256.txt.asc`,
				),
		...
		if (platform !== "win32")
			await exec(
				`grep "${path.basename(tarball)}" "${shasums}" | shasum -a 256 -c -`,
				{ cwd: basedir },
			);
	};
	...
	const extract = async () => {
		...
		if (platform === "win32") {
			...
			await move(
				path.join(nodeTmp, nodeBase, "node.exe"),
				path.join(cache, "node.exe"),
			);
		} else {
			await exec(`tar -C "${tmp}/node" -xJf "${tarball}"`);
			await move(
				path.join(nodeTmp, nodeBase, "bin", "node"),
				path.join(cache, "node"),
			);
		}
	};
