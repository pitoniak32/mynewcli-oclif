...
export async function buildConfig(
	...
	const nodeOptions = castArray(
		(updateConfig.node ?? ({} as { options?: string | string[] })).options ??
			[],
	);
	const targets = compact(
		options.targets || updateConfig.node?.targets || TARGETS,
	)
		.filter((t) => {
			if (t === "darwin-arm64" && semver.lt(nodeVersion, "16.0.0")) {
				ux.warn(
					"darwin-arm64 is only supported for node >=16.0.0. Skipping...",
				);
				return false;
			}
		...
		.map((t) => {
			const [platform, arch] = t.split("-") as [
				Interfaces.PlatformTypes,
				Interfaces.ArchTypes,
			];
	...
	return {
		...
		workspace(target) {
			...
			if (target && target.platform)
				return path.join(
					base,
					[target.platform, target.arch].join("-"),
					templateShortKey("baseDir", { bin: config.bin }),
				);
